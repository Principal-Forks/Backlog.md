...
export async function initializeProject(
	...
	try {
		...
		span.addEvent("init.config.saved", {
			projectName,
		...
		if (Object.keys(mcpResults).length > 0) {
			span.addEvent("init.mcp.configured", {
				clientCount: Object.keys(mcpResults).length,
			});
		}

		span.addEvent("init.complete", {
			projectName,
			success: true,
		});
